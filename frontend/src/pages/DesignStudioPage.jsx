...
import { compareRuleOrder, inferValidationDomain, parseRuleConfig } from "./validationRulesUtils";
import {
  buildSubstituteGroupsFromRows,
  computeBasketValidationErrors,
  normalizeCoursePair,
  sanitizeBasketSubGroupRows,
} from "./basketRulesUtils";
...
export function DesignStudioPage() {
  ...
  function deriveBasketSubstituteRows(basketId, courseIds, requirementId) {
    ...
    const reqRows = (requirementSubstitutionsVersionQ.data || []).filter(
      (s) => s.requirement_id === reqId
    );
    const rows = basketRows.length ? basketRows : reqRows;
    return buildSubstituteGroupsFromRows(rows, courseIds, courseMapById);
  }
  ...
  useEffect(() => {
    ...
    setBasketSubGroupRows((prev) => {
      if (prev.length) {
        return sanitizeBasketSubGroupRows(prev, ids);
      }
  ...
  async function saveRequirementBasketModal() {
    ...
    for (const row of relevantExisting) {
      const key = normalizeCoursePair(row.primary_course_id, row.substitute_course_id);
    ...
    for (const row of desiredRows) {
      for (const sid of row.substitute_course_ids || []) {
        desiredNorm.add(normalizeCoursePair(row.primary_course_id, sid));
      }
    }
    ...
    try {
      ...
      for (const row of desiredRows) {
        for (const sid of row.substitute_course_ids || []) {
          const key = normalizeCoursePair(row.primary_course_id, sid);
  ...
  function openCoreRulesBuilder(programIdArg, programNameArg) {
    ...
    const reqMeta = buildCoreRuleRequirementMeta(optionalCoreReqs, coreRuleChoiceOptionsByReq);
    const rows = buildInitialCoreRuleRows(optionalCoreReqs, reqMeta); // all-required substitute groups remain opt-in
  ...
  const basketValidationErrors = useMemo(() => {
    return computeBasketValidationErrors({
      reqLinkKind,
      basketCourseIds,
      basketMinCount,
      basketSelectedId,
