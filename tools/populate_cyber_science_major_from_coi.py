...
POLICY_ELECTIVE_OPTIONS = ["Law 440", "MSS 444", "Pol Sci 466"]
...
def recreate_cyber_core_rules_validation_rule(db, version_id: str, program: AcademicProgram, course_id_by_num: dict[str, str]) -> None:
    ...
    for vr in existing:
        ...
        if pid == program.id or pname in {program.name.lower(), "cyber science"}:
            db.delete(vr)
    db.flush()
    ...
    db.add(
        ValidationRule(
            name="Program Pathway - Cyber Science",
            tier=2,
            severity="FAIL",
            active=True,
...
def main() -> None:
    with SessionLocal() as db:
        ...
        technical_ids = optional_course_ids(course_id_by_num, TECHNICAL_ELECTIVE_OPTIONS)
        policy_ids = optional_course_ids(course_id_by_num, POLICY_ELECTIVE_OPTIONS)
        open_ids = list(dict.fromkeys([*technical_ids, *policy_ids]))
        ...
        option_nodes = [
            ...
            ("Track - Cyber Science Technical Elective: Any One", technical_ids, "ANY_ONE", None, 2),
            ("Track - Cyber Science Policy Elective: Any One", policy_ids, "ANY_ONE", None, 3),
            ("Track - Cyber Science Open Elective: Any One", open_ids, "ANY_ONE", None, 4),
        ]
        for node_name, ids, logic_type, pick_n, sort_order in option_nodes:
            ...
            basket = create_basket_with_items(
                db,
                version_id=version_id,
                name=f"Cyber Science Major - {node_name} Options",
                description=f"COI 2025-2026 {PROGRAM_NAME}: {node_name}",
                course_ids=ids,
            )
            attach_basket(db, req.id, basket.id, min_count=1)

        recreate_cyber_core_rules_validation_rule(db, version_id, program, course_id_by_num)

        db.commit()
        print(f"Populated {PROGRAM_NAME} in version {active.name} ({version_id})")
        print("Created structure:")
        print("- Major Requirement: All Required (11 mandatory Cyber Science courses)")
        print("- Math Foundation Option: Any One")
        print("- Cyber Science Technical Elective: Any One")
        print("- Cyber Science Policy Elective: Any One")
        print("- Cyber Science Open Elective: Any One")
        print("Added/updated major-specific Core Rules validation rule: Program Pathway - Cyber Science")
