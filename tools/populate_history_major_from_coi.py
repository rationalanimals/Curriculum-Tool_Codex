...
REQUIRED = ["History 200", "History 210", "History 220", "History 400", "History 401"]
AIRPOWER = ["History 320", "History 322"]
REGIONAL = ["History 230", "History 240", "History 250", "History 260", "History 270", "History 280", "History 290"]
SOCIOCULTURAL = ["History 331", "History 341", "History 347", "History 364", "History 366", "Hum 430"]
...
def main() -> None:
    with SessionLocal() as db:
        ...
        req_ids = optional_course_ids(cmap, REQUIRED)
        air_ids = optional_course_ids(cmap, AIRPOWER)
        reg_ids = optional_course_ids(cmap, REGIONAL)
        soc_ids = optional_course_ids(cmap, SOCIOCULTURAL)

        hist_option_ids = []
        for c in all_courses:
            num = str(c.course_number or "")
            if num.startswith("History "):
                m = re.search(r"(\d{3})", num)
                if m and int(m.group(1)) >= 300:
                    hist_option_ids.append(c.id)
        hum430 = cmap.get(normalize_course_number("Hum 430"))
        if hum430:
            hist_option_ids.append(hum430)
        for c in all_courses:
            num = str(c.course_number or "")
        ...
        humanities_option_ids = []
        for c in all_courses:
            num = str(c.course_number or "")
            if num.startswith(("English ", "Creat Art ", "History ", "Hum ", "For Lang ", "Philos ")):
                if float(c.credit_hours or 0.0) >= 3.0:
                    humanities_option_ids.append(c.id)

        root = create_requirement(
            db, version_id=version_id, parent_requirement_id=None, program_id=p.id, name="Major - History",
        ...
        link_all_required(db, r_fixed.id, req_ids)

        def add_anyone(name: str, sort_order: int, course_ids: list[str], label: str):
            ...
            bb = create_basket(db, version_id, f"History Major - {label} Pool", f"COI 2025-2026 History {label}", course_ids)
            attach_basket(db, rr.id, bb, 1)

        add_anyone("Track - Airpower History Option: Any One", 1, air_ids, "Airpower History Option")
        add_anyone("Track - Regional History Option: Any One", 2, reg_ids, "Regional History Option")
        add_anyone("Track - Socio-Cultural History Option: Any One", 3, soc_ids, "Socio-Cultural History Option")

        r_hist_opts = create_requirement(
            db, version_id=version_id, parent_requirement_id=root.id, program_id=p.id,
            name="Track - History Options: Pick N", logic_type="PICK_N", pick_n=5, sort_order=4, category="MAJOR",
            major_mode="TRACK", track_name="History Options"
        )
        b_hist = create_basket(db, version_id, "History Major - History Options Pool", "COI 2025-2026 History options", hist_option_ids)
        attach_basket(db, r_hist_opts.id, b_hist, 5)

        add_anyone("Track - Humanities Option: Any One", 5, humanities_option_ids, "Humanities Option")
